...
async function uploadImage(imageBase64, apiKey) {
  ...
  let data;
  try { data = JSON.parse(bodyText); } catch { throw new Error(`Upload gagal: response bukan JSON - ${bodyText.slice(0, 100)}`); }
...
async function startGeneration(prompt, imageBase64, options) {
  ...
  if (imageBase64) {
    try {
      imageUrl = await uploadImage(imageBase64, apiKey);
    } catch (uploadErr) {
      console.warn("Pollinations upload failed, falling back to T2V:", uploadErr.message);
    }
  }
