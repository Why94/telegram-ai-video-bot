...
async function uploadImage(imageBase64, apiKey) {
...
async function submitTask(prompt, imageBase64 = null, options = {}) {
  ...
  if (!apiKey) {
    return { success: false, error: "Leonardo AI API Key tidak tersedia untuk akun ini." };
  }

  const model = options.model || config.PROVIDERS.leonardo.defaultModel;
  ...
  if (imageBase64) {
    try {
      const imageId = await uploadImage(imageBase64, apiKey);
      params.guidances = {
        start_frame: [
          {
            image: {
...
async function pollTask(taskId, apiKey) {
  if (!apiKey) {
    return { status: "error", error: "API key diperlukan untuk poll Leonardo." };
  }
  try {
