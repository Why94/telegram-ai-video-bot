...
async function pollTask(taskId) {
  const providerConfig = config.PROVIDERS.kling;
  const endpoint = `${providerConfig.apiBase}/v1/videos/${taskId}`;
  ...
  try {
    ...
    let status = "unknown";
    if (taskData.task_status === "succeed" || taskData.status === "succeeded" || taskData.status === "completed") {
      status = "succeeded";
    } else if (taskData.task_status === "processing" || taskData.status === "processing") {
      status = "running";
    } else if (taskData.task_status === "submitted" || taskData.status === "submitted" || taskData.status === "queued" || taskData.status === "created") {
      status = "queued";
