...
function findUrlInResponse(obj) {
  if (typeof obj === "string") {
    return (obj.startsWith("http://") || obj.startsWith("https://")) ? obj : null;
  }
  if (!obj || typeof obj !== "object") return null;
  for (const key of ["fileUrl", "url", "file_url", "videoUrl", "imageUrl", "downloadUrl", "path", "data"]) {
...
async function uploadImage(apiKey, base64DataUri) {
  ...
  throw new Error((data.msg || data.message || "Image upload succeeded but no URL found") + " | response: " + JSON.stringify(data).substring(0, 500));
}
