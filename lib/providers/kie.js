...
async function uploadImage(apiKey, base64DataUri) {
  ...
  const fileUrl = data.data?.fileUrl || data.data?.url || data.data?.file_url || data.url || data.fileUrl;
  if (fileUrl) {
    return fileUrl;
  }
  throw new Error(data.msg || data.message || "Image upload failed: no URL in response");
}
