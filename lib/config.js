...
const PROVIDERS = {
  ...
  freetheai: {
    ...
    usable: false,
    statusNote: "Sementara dinonaktifkan — hanya Leonardo AI yang tersedia",
...
const config = {
  ...
  defaultProvider: process.env.DEFAULT_PROVIDER || "leonardo",
  ...
  // Only these account product names are shown/sold to clients.
  // Empty = show all. Set to restrict the shop to specific products.
  ALLOWED_ACCOUNT_PRODUCTS: (process.env.ALLOWED_ACCOUNT_PRODUCTS || "Leonardo AI")
    .split(",").map((s) => s.trim()).filter(Boolean),

  // Credit cost (per generation) for Leonardo AI image-to-video.
  LEONARDO_CREDIT_COST: parseInt(process.env.LEONARDO_CREDIT_COST || "2000", 10),

  // Low stock warning threshold (notify admins when available stock drops below this)
  LOW_STOCK_THRESHOLD: parseInt(process.env.INVENTORY_LOW_STOCK || "10", 10),

  // Manual payment (used while Xendit business verification is pending).
  // Set via env MANUAL_PAYMENT_INFO (JSON) or use these defaults.
  MANUAL_PAYMENT: (() => {
    try {
      if (process.env.MANUAL_PAYMENT_INFO) return JSON.parse(process.env.MANUAL_PAYMENT_INFO);
    } catch (e) {}
    return {
      ratePerCredit: parseInt(process.env.CREDIT_RATE_IDR || "1000", 10), // Rp per credit
      methods: [
        { name: "DANA", number: process.env.MANAUAL_DANA || "08xxxxxxxxx" },
        { name: "OVO", number: process.env.MANUAL_OVO || "08xxxxxxxxx" },
        { name: "BCA", number: process.env.MANUAL_BCA || "xxxxxxxxxx" },
      ],
      adminContact: process.env.MANUAL_PAYMENT_ADMIN || "@admin_username",
      note: "Kirim bukti transfer setelah bayar. Saldo masuk setelah admin konfirmasi (maks 1 jam).",
    };
  })(),
