...
async function importAccounts({ buffer, filename, mime, duplicateMode, adminUserId }) {
  ...
  const insertSql =
    ...
    "profile_name,country,plan,expire_date,notes,status) VALUES (?,?,?,?,?,?,?,?,?,?,?)";
  ...
  for (let i = 0; i < parsed.valid.length; i += batchSize) {
    ...
    for (const rec of chunk) {
      ...
      run(db, insertSql, [
        ...
        a.profile_name, a.country, a.plan, a.expire_date, a.notes, a.status,
      ]);
      imported++;
    }
  }
...
function exportAccounts({ format = "csv", filters = {}, ids = null }) {
  ...
  const data = rows.map((r) => ({
    ...
    order_id: r.order_id, telegram_user_id: r.telegram_user_id,
  }));
