...
// Returns { id, apiKey } or null. The account is NOT sold — it's reused for generation.
function reserveLeonardoAccount(productName) {
  const db = dbMod.getDb();
  const acc = qOne(db,
  ...
  if (!acc) return null;
  run(db,
    "UPDATE accounts SET status='IN_USE', updated_at=datetime('now') WHERE id=?",
    [acc.id]);
  dbMod.audit("LEONARDO_RESERVE", null, "account", String(acc.id), { productName });
...
// ─── User Credits ────────────────────────────────────────────────────────────
function ensureUser(userId, { username = null, first_name = null } = {}) {
  const db = dbMod.getDb();
  const existing = qOne(db, "SELECT * FROM users WHERE user_id=?", [String(userId)]);
  if (!existing) {
    run(db,
  ...
  if (username || first_name) {
    runN(db, "UPDATE users SET username=COALESCE(@u,username), first_name=COALESCE(@f,first_name), updated_at=datetime('now') WHERE user_id=@id",
...
function getCredit(userId) {
  const db = dbMod.getDb();
  const u = qOne(db, "SELECT credit FROM users WHERE user_id=?", [String(userId)]);
  return u ? u.credit : 0;
}

function addCredit(userId, amount, { adminUserId = null, reason = null } = {}) {
  const db = dbMod.getDb();
  ensureUser(userId);
  runN(db, "UPDATE users SET credit = credit + @amt, updated_at=datetime('now') WHERE user_id=@id",
    { amt: amount, id: String(userId) });
  dbMod.audit("ADD_CREDIT", adminUserId, "user", String(userId), { amount, reason });
...
function purchaseWithCredit(productName, { telegramUserId, username = null, first_name = null, price = null }) {
  ensureUser(telegramUserId, { username, first_name });
  // resolve price: from arg or from an AVAILABLE account of this product
  let unitPrice = price;
  if (unitPrice == null) {
    const db = dbMod.getDb();
    const row = qOne(db, "SELECT price FROM accounts WHERE product_name=? AND status='AVAILABLE' AND price IS NOT NULL LIMIT 1", [productName]);
  ...
  if (unitPrice > 0) {
    const bal = getCredit(telegramUserId);
    if (bal < unitPrice) {
  ...
  if (unitPrice > 0) deductCredit(telegramUserId, unitPrice);
  const delivered = deliverAccount(reserved.id, { orderId, telegramUserId });
  dbMod.audit("PURCHASE_CREDIT", null, "account", String(reserved.id), { productName, price: unitPrice, orderId });
  return { success: true, account: delivered, price: unitPrice, balance: getCredit(telegramUserId) };
}
