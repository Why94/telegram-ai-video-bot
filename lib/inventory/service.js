...
function isDbArg(a) {
  return a && typeof a === "object" && !(a instanceof String) && !Array.isArray(a);
}
function extractNamed(args) {
  // legacy: qN(db, sqlNamed, namedParams)  |  new: qN(sqlNamed, namedParams)
  if (args.length >= 2 && isDbArg(args[0])) {
    return { sqlNamed: args[1], namedParams: args[2] || {} };
  }
  return { sqlNamed: args[0], namedParams: args[1] || {} };
}
async function qN(...args) {
  const { sqlNamed, namedParams } = extractNamed(args);
